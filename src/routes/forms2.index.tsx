...
import type { PGStatus } from '@/types/forms2-announcement'
import type { PGFilters } from '@/components/forms2/pg-filter-bar'
import { useSetBreadcrumbs } from '@/hooks/use-breadcrumbs'
import { mockForms2Announcements } from '@/data/mock-forms2-announcements'
import { PGStatusBadge } from '@/components/forms2/pg-status-badge'
import { PGReadRate } from '@/components/forms2/pg-read-rate'
import {
  EMPTY_PG_FILTERS,
  PGFilterBar,
} from '@/components/forms2/pg-filter-bar'
...
function Forms2Page() {
  ...
  useSetBreadcrumbs([{ label: 'Announcements', href: '/forms2' }])
  const navigate = useNavigate()

  const filtered = useMemo(() => {
    return mockForms2Announcements
      .filter((a) => {
  ...
  const metrics = useMemo(() => {
    const total = mockForms2Announcements.length
    const drafts = mockForms2Announcements.filter(
      (a) => a.status === 'draft',
    ).length
    const posted = mockForms2Announcements.filter(
      (a) => a.status === 'posted',
    ).length
    const scheduled = mockForms2Announcements.filter(
      (a) => a.status === 'scheduled',
    ).length
    const postedAnnouncements = mockForms2Announcements.filter(
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 pt-6">
        ...
        <Button render={<Link to="/forms2/new" />}>
