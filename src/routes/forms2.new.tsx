...
import type {
  ...
  PGRecipient,
  Shortcut,
} from '@/types/forms2-announcement'
import type { FormQuestion, ReminderType, ResponseType } from '@/types/form'
import type { SelectedEntity } from '@/components/forms2/entity-selector'
import { QuestionBuilder } from '@/components/forms/question-builder'
import { useSetBreadcrumbs } from '@/hooks/use-breadcrumbs'
import { StudentRecipientSelector } from '@/components/forms2/student-recipient-selector'
import { SendConfirmationSheet } from '@/components/forms2/send-confirmation-sheet'
import { RichTextArea } from '@/components/forms2/rich-text-area'
import { PGShortcutsSelector } from '@/components/forms2/pg-shortcuts-selector'
import { StaffSelector } from '@/components/forms2/staff-selector'
import { EnquiryEmailSelector } from '@/components/forms2/enquiry-email-selector'
import { mockStudents } from '@/data/mock-students'
import {
  getForms2AnnouncementById,
  mockForms2Announcements,
} from '@/data/mock-forms2-announcements'
...
export const Route = createFileRoute('/forms2/new')({
...
function NewAnnouncementPage() {
  ...
  const existingAnnouncement = editId
    ? getForms2AnnouncementById(editId)
    : undefined

  useSetBreadcrumbs([
    { label: 'Announcements', href: '/forms2' },
    {
  ...
  function handleConfirmPost() {
    ...
    if (isEditing && editId) {
      ...
      const idx = mockForms2Announcements.findIndex((a) => a.id === editId)
      if (idx !== -1) {
        mockForms2Announcements[idx] = {
          ...mockForms2Announcements[idx],
      ...
      navigate({ to: '/forms2/$id', params: { id: editId } })
    } else {
      mockForms2Announcements.unshift(newAnnouncement)
      setShowConfirmSheet(false)
      toast.success(
