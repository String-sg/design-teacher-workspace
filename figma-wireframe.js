...
;(async () => {
  ...
  all.push(sidebar)
  all.push(
    await txt('Teacher Workspace', 16, 14, 12, 'SemiBold', {
      r: 0.9,
      g: 0.9,
      b: 0.9,
    }),
  ...
  for (let i = 0; i < navLabels.length; i++) {
    ...
    if (active)
      all.push(
        box(
          `Nav Active – ${navLabels[i]}`,
          8,
          y - 2,
          204,
          30,
          { r: 0.25, g: 0.27, b: 0.35 },
          6,
        ),
      )
    all.push(
      box(
        `Nav Icon – ${navLabels[i]}`,
        16,
        y + 4,
        16,
        16,
    ...
    all.push(
      await txt(
        ...
        active ? { r: 0.95, g: 0.95, b: 0.95 } : { r: 0.55, g: 0.55, b: 0.6 },
      ),
    )
  }
  ...
  all.push(
    await txt('Profiles', 236, 13, 13, 'SemiBold', {
      r: 0.85,
      g: 0.85,
      b: 0.85,
    }),
  ...
  // page header
  all.push(
    await txt('Profiles', 236, 56, 18, 'Bold', { r: 0.85, g: 0.85, b: 0.85 }),
  )
  all.push(
    await txt(
      'You can view and manage student profiles.',
      236,
      80,
      11,
      'Regular',
  ...
  all.push(
    await txt('Secondary 3', 236, 104, 16, 'SemiBold', {
      r: 0.85,
      g: 0.85,
      b: 0.85,
    }),
  ...
  // table header row
  all.push(
    box('Table Header', 220, 218, 1060, 36, { r: 0.16, g: 0.16, b: 0.2 }),
  )
  const cols = [
    'Name',
    'Class',
  ...
  for (let i = 0; i < cols.length; i++) {
    all.push(
      await txt(cols[i], colX[i], 227, 11, 'Medium', {
        r: 0.55,
        g: 0.55,
        b: 0.6,
      }),
  ...
  for (let r = 0; r < 8; r++) {
    const rowY = 256 + r * 44
    all.push(
      box(`Row Divider ${r}`, 220, rowY, 1060, 1, { r: 0.2, g: 0.2, b: 0.24 }),
    )
    all.push(
      box(
        `Row Avatar ${r}`,
  ...
  // OVERLAY SCRIM
  // ════════════════════════════════════════════════════════════════════════
  const scrim = box('Overlay Scrim', 0, 0, 1280, 800, {
    r: 0.05,
    g: 0.05,
    b: 0.08,
  })
  ...
  all.push(closeBtn)
  all.push(
    await txt('✕', mx + mw - 36, my + 22, 12, 'Regular', {
      r: 0.4,
      g: 0.4,
      b: 0.4,
    }),
  ...
  // divider
  const divider1 = box('Modal Divider', mx, my + 56, mw, 1, {
    r: 0.9,
    g: 0.9,
    b: 0.9,
  })
  ...
  function checkbox(x, y, checked) {
    const cb = box(
      ...
      16,
      16,
      checked ? { r: 0.25, g: 0.47, b: 0.95 } : { r: 1, g: 1, b: 1 },
      4,
    )
    cb.strokes = [
      {
  ...
  all.push(
    await txt(
      'Export data as shown in the table',
      mx + 62,
      my + 124,
      11,
      'Regular',
  ...
  all.push(
    await txt('Redacted', mx + 62, my + 175, 13, 'Regular', {
      r: 0.3,
      g: 0.3,
      b: 0.3,
    }),
  ...
  all.push(
    await txt(
      'Export with sensitive-high data masked',
      mx + 62,
      my + 192,
      11,
      'Regular',
  ...
  all.push(exportBtn)
  all.push(
    await txt('Export view', mx + mw - 126, my + mh - 42, 13, 'SemiBold', {
      r: 1,
      g: 1,
      b: 1,
    }),
