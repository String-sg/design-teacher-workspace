...
import { RecipientReadTable } from '~/apps/pg/components/comms/recipient-read-table';
import { StatusBadge } from '~/apps/pg/components/comms/status-badge';
import { getPGAnnouncementById } from '~/apps/pg/data/mock-pg-announcements';
import { CLASS_GROUPS } from '~/apps/pg/data/mock-student-groups';
...
function AnnouncementDetailPage() {
  ...
  const readCount = savedData.recipients.filter((r) => r.readStatus === 'read').length;
  ...
  const respondedCount = savedData.recipients.filter((r) => r.respondedAt != null).length;
  ...
  const postedDate = announcement.postedAt ? formatDateTime(announcement.postedAt) : null;
  const scheduledDate = savedData.scheduledAt ? formatDateTime(savedData.scheduledAt) : null;
  ...
  function startEditingSchedule() {
    const current = savedData.scheduledAt ? new Date(savedData.scheduledAt) : new Date();
    const pad = (n: number) => String(n).padStart(2, '0');
    setEditScheduleDate(
      `${current.getFullYear()}-${pad(current.getMonth() + 1)}-${pad(current.getDate())}`,
    );
    setEditScheduleTime(`${pad(current.getHours())}:${pad(current.getMinutes())}`);
        ...
        <div className="flex items-start justify-between gap-4">
                ...
                (isEditingSchedule ? (
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <input
                      ...
                      className="rounded-md border border-slate-300 bg-white px-2.5 py-1 text-sm text-foreground outline-none focus:ring-2 focus:ring-blue-300"
                    />
                    <Button size="sm" onClick={saveSchedule} disabled={!editScheduleDate}>
                      Save
                    </Button>
                    <button
                      type="button"
          ...
          {isViewer ? (
            ...
            <Button
              variant="default"
              size="sm"
              render={<Link to="/announcements/new" search={{ edit: announcement.id }} />}
            >
              Edit
            </Button>
          ) : (
            ...
            <div className="rounded-lg border bg-white p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {unreadCount > 0 ? (
                    <p className="mt-1 text-sm text-amber-600">{unreadCount} unread</p>
                  ) : (
            ...
            <div className="rounded-lg border bg-white p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {pendingAckCount > 0 ? (
                    <p className="mt-1 text-sm text-amber-600">{pendingAckCount} pending</p>
                  ) : (
            ...
            <div className="flex h-40 items-center justify-center rounded-lg border bg-white">
              <div className="flex flex-col items-center gap-2 text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Read tracking will be available after the announcement is sent.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              {isEditing && !contentLocked ? (
                <div className="space-y-1">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Formatting is simplified here. Use the compose form for rich-text editing.
                  </p>
                </div>
              ) : (
              ...
              {isEditing && contentLocked && (
                <p className="flex items-center gap-1.5 rounded-md bg-slate-50 px-3 py-2 text-xs text-muted-foreground">
                  ...
                  Title, description, and shortcuts cannot be edited after posting.
                </p>
              )}
              ...
              {announcement.shortcuts.length > 0 && (
                ...
                >
                  <p className="mb-1.5 text-xs font-medium text-muted-foreground">Shortcuts</p>
                  {PG_SHORTCUT_PRESETS.filter((preset) =>
                    ...
                    >
                      ...
                      <p className="text-sm font-medium text-blue-900">{preset.composerLabel}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {announcement.attachments && announcement.attachments.length > 0 && (
                <div className="space-y-1.5 border-t pt-3">
                  <p className="mb-1.5 text-xs font-medium text-muted-foreground">Attachments</p>
                  {announcement.attachments.map((att, i) => (
                    <div key={i} className="flex items-center gap-2 text-sm">
                      <Paperclip className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                      <span className="min-w-0 truncate text-sm">{att.name}</span>
              ...
              {isEditing ? (
                <div className="space-y-1 border-t pt-3">
                  ...
                  <div className="relative">
                    <Mail className="absolute top-1/2 left-3 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
                    <Input
                      type="email"
              ...
              <div className="border-t pt-3">
                {isEditing ? (
                  <div className="space-y-1">
                    <p className="text-xs font-medium text-muted-foreground">Staff in charge</p>
                    <div className="relative">
                  ...
                  announcement.staffInCharge && (
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-sm font-medium">{announcement.staffInCharge}</p>
                    </div>
                  )
                )}
              </div>
