import {
  ...
  STUDENT_OVERLAP_MAP,
} from '~/apps/pg/data/mock-student-groups';

import type { SearchResults, SelectedEntity } from './entity-selector';
import { detectOverlaps, EntitySelector } from './entity-selector';
...
function studentSearchFn(query: string): SearchResults {
  ...
  return {
    groups: ALL_STUDENT_GROUP_ITEMS.filter((g) => g.label.toLowerCase().includes(q)),
...
export function StudentRecipientSelector({ value, onChange }: StudentRecipientSelectorProps) {
