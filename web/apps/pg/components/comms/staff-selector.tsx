import { MOCK_STAFF, MOCK_STAFF_GROUPS } from '~/apps/pg/data/mock-staff';
...
function makeMemberNames(ids: string[]): string[] {
  return ids
    .map((id) => MOCK_STAFF.find((s) => s.id === id)?.name)
    .filter((n): n is string => Boolean(n));
}

function makeMemberDetails(ids: string[]): MemberDetail[] {
...
const STAFF_INDIVIDUAL_ITEMS: EntityItem[] = MOCK_STAFF.map((s) => ({
  id: s.id,
  label: s.name,
  sublabel: [s.formClass, s.email].filter(Boolean).join(' · '),
...
const LEVEL_GROUP_ITEMS: EntityItem[] = MOCK_STAFF_GROUPS.filter(
  (g) => g.groupType === 'level',
).map((g) => ({
  id: g.id,
  label: g.label,
...
const SCHOOL_GROUP_ITEMS: EntityItem[] = MOCK_STAFF_GROUPS.filter(
...
const ALL_STAFF_GROUP_ITEMS: EntityItem[] = [...LEVEL_GROUP_ITEMS, ...SCHOOL_GROUP_ITEMS];

// Scopes: Individual | Level | School — mirrors PG app's staff-in-charge picker
const STAFF_SCOPES: EntityScope[] = [
...
function staffSearchFn(query: string): SearchResults {
  ...
  return {
    groups: ALL_STAFF_GROUP_ITEMS.filter((g) => g.label.toLowerCase().includes(q)),
